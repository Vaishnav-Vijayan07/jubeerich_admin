...
const BasicInputElements = withSwal((props: any) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: "status",
      sort: true,
      Cell: ({ row }: any) => <span>{row.original.status ? "active" : "disabled"}</span>,
    },
    {
      Header: "Actions",
      accessor: "",
      ...
      Cell: ({ row }: any) => (
        <div className="d-flex justify-content-center align-items-center gap-2">
          ...
          <FeatherIcons
            ...
            size="15"
            className="cursor-pointer text-secondary"
            onClick={() => handleDelete(row.original.id, row.original.updated_by)}
          />
        </div>
      ),
    },
...
const Category = () => {
  ...
  const { state, loading, success, error } = useSelector((state: RootState) => ({
  ...
  return (
    <React.Fragment>
      ...
      <Row>
        <Col>
          <BasicInputElements state={state} loading={loading} success={success} error={error} />
        </Col>
      </Row>
    </React.Fragment>
  );
